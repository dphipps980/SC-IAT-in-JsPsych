//Touch-screen version of qstiat6.js (Qualtrics ST-IAT).
//Created from baranan/minno-tasks qstiat6.js, with the touch-input mechanism
//ported over from the Qualtrics IAT API (IATAPITouch.js / iat8.js).
//Set isTouch:false to get the original keyboard task back.
define(['pipAPI','pipScorer','underscore'], function(APIConstructor, Scorer, _) {
	...
	function stiatExtension(options)
	{
		...
		var stiatObj =
		{
			...
			finalTouchText: 'You have completed this task<br/><br/>Touch the bottom green area to continue.',

			/**** Touch-specific settings (copied from the touch IAT) ****/
			touchMaxStimulusWidth : '50%', //Max width of a target stimulus on touch.
			touchMaxStimulusHeight : '50%', //Max height of a target stimulus on touch.
			bottomTouchCss: {}, //Add any CSS value you want for changing the css of the bottom touch area.
			//The two green response areas. Geometry and colour match the touch IAT.
			touchAreaSize : {height:48, width:30},
			touchAreaCss : {background:'#00FF00', opacity:0.3, zindex:-1},

			instWidth : 99, //The width of the instructions stimulus.
			//Where the instructions text starts on touch (percent from the top of the canvas).
			//The category labels are drawn above it, so these leave room for them.
			//Tune these two if the text and the labels collide on your device.
			touchInstTopPractice : 12, //Practice blocks show two labels only.
			touchInstTopCombined : 25, //Combined blocks also show "or" and the category label.
			//Style of the instructions text on touch. Smaller than the desktop text
			//because a phone canvas is short.
			touchInstCss : {'font-size':'1.1em', color:'black', lineHeight:1.15},
			//Vertical position of the labels on touch. The key reminders ("Press E for")
			//are not drawn on touch, so the labels move up.
			touchLabelTop : 2,
			...
			//Touch versions of the three instructions templates above.
			//Same replaced variables: blockNum, nBlocks, attribute1, attribute2, thecategory.
			instTemplatePracticeTouch : [
				'<div>',
					'<p align="center">',
						'<u>Part blockNum of nBlocks</u>',
					'</p>',
					'<p align="left" style="margin-left:5px">',
						'Put a left finger over the <b>left</b> green area for items that belong to the category <font color="#31b404">attribute1</font>.<br/>',
						'Put a right finger over the <b>right</b> green area for items that belong to the category <font color="#31b404">attribute2</font>.<br/>',
			...
			instTemplateCategoryRightTouch : [
				'<div>',
					'<p align="center">',
						'<u>Part blockNum of nBlocks</u>',
					'</p>',
					'<p align="left" style="margin-left:5px">',
						'Touch the <b>left</b> green area for items that belong to the category <font color="#31b404">attribute1</font>.<br/>',
						'Touch the <b>right</b> green area for items that belong to the category <font color="#31b404">attribute2</font> ',
						'and for items that belong to the category <font color="#31b404">thecategory</font>.<br/>',
			...
			instTemplateCategoryLeftTouch : [
				'<div>',
					'<p align="center">',
						'<u>Part blockNum of nBlocks</u>',
					'</p>',
					'<p align="left" style="margin-left:5px">',
						'Touch the <b>left</b> green area for items that belong to the category <font color="#31b404">attribute1</font> ',
						'and for items that belong to the category <font color="#31b404">thecategory</font>.<br/>',
						'Touch the <b>right</b> green area for items that belong to the category <font color="#31b404">attribute2</font>.<br/>',
						'Each item belongs to only one category.<br/>',
						'If you make a mistake, a red <font color="#ff0000"><b>X</b></font> will appear. Touch the other side. <u>Go as fast as you can</u> while being accurate.',
					'</p>',
					'<p align="center">Touch the <b>lower</b> green area to start.</p>',
		...
		************************************************************************************/
		...
		if (isTouch)
		{//Keep the target stimuli small enough to leave the green areas reachable.
			var maxW = piCurrent.touchMaxStimulusWidth;
			var maxH = piCurrent.touchMaxStimulusHeight;
			piCurrent.attribute1.css.maxWidth = maxW;
			piCurrent.attribute2.css.maxWidth = maxW;
			piCurrent.category.css.maxWidth = maxW;
			piCurrent.attribute1.css.maxHeight = maxH;
			piCurrent.attribute2.css.maxHeight = maxH;
			piCurrent.category.css.maxHeight = maxH;
		}

		/**
		 * Create inputs.
		 * On touch, 'left'/'right' become taps inside the green response areas
		 * (matched by their data-handle), and 'space' becomes the bottom green bar.
		 */
		...
		//The "or" and the category label sit this far below the attribute label.
		var att1Height = (piCurrent.attribute1.title.height|3);
		var att2Height = (piCurrent.attribute2.title.height|3);
		//On touch we drop the key reminders, so everything moves up.
		var labelTop = isTouch ? piCurrent.touchLabelTop : 4;

		//The key reminder row ("Press E for" / "Press I for"). Not shown on touch.
		var keyReminders = isTouch ? [] : [
		...
		//On touch, centre each label over its own green area rather than at a fixed inset.
		function labelLoc(side, top)
		{
			var loc = {top:top};
			loc[side] = isTouch ? 0 : 6;
			return loc;
		}
		//On touch, constrain the label to the width of its green area so it wraps inside it.
		function labelSize()
		{
			return isTouch ? {width:piCurrent.touchAreaSize.width} : undefined;
		}
		function labelStim(side, top, media, css)
		{
			var stim = {location:labelLoc(side, top), media:media, css:css};
			var size = labelSize();
			if (size) {stim.size = size;}
			return stim;
		}

		// layout object for the trials where category on left
		var leftLayout = keyReminders.concat([
			labelStim('left', labelTop, piCurrent.attribute1.title.media, piCurrent.attribute1.title.css),
			labelStim('right', labelTop, piCurrent.attribute2.title.media, piCurrent.attribute2.title.css),
			labelStim('left', labelTop+att1Height, {word:piCurrent.orText}, piCurrent.orCss),
			labelStim('left', labelTop+7+att1Height, piCurrent.category.title.media, piCurrent.category.title.css)
		]);
		// layout object for the trials where category on right
		var rightLayout = keyReminders.concat([
			labelStim('left', labelTop, piCurrent.attribute1.title.media, piCurrent.attribute1.title.css),
			labelStim('right', labelTop, piCurrent.attribute2.title.media, piCurrent.attribute2.title.css),
			labelStim('right', labelTop+att2Height, {word:piCurrent.orText}, piCurrent.orCss),
			labelStim('right', labelTop+7+att2Height, piCurrent.category.title.media, piCurrent.category.title.css)
		]);
		// layout object for practice blocks (no category)
		var pracLayout = keyReminders.concat([
			labelStim('left', labelTop, piCurrent.attribute1.title.media, piCurrent.attribute1.title.css),
			labelStim('right', labelTop, piCurrent.attribute2.title.media, piCurrent.attribute2.title.css)
		]);

		var reminderStimulus = 	{location:{bottom:1}, css: {color:piCurrent.fontColor,'font-size':'1em'},
			media : {html: isTouch ? piCurrent.remindErrorTextTouch : piCurrent.remindErrorText}};

		//The two green response areas, added to sorting trials only (not to instructions,
		//where the bottom green bar is the active control instead).
		//They are appended last so they paint over the labels, which means a tap on a
		//label still counts as a tap on that side.
		var touchAreas = !isTouch ? [] : [
			{inherit:{type:'byData', set:'touchInputStimuli', data:{handle:'right'}}},
			{inherit:{type:'byData', set:'touchInputStimuli', data:{handle:'left'}}}
		];
		//Everything a sorting trial needs on top of the block's labels.
		var trialExtras = [reminderStimulus].concat(touchAreas);
		...
		API.addStimulusSets({
			...
			instructions: [
				isTouch
				? {css:piCurrent.touchInstCss, nolog:true, location:{left:0,top:0}, size:{width:piCurrent.instWidth}}
			...
			//The two green response areas used on touch. Same geometry and colour as the touch IAT.
			touchInputStimuli: [
				{media:{html:'<div></div>'}, size:piCurrent.touchAreaSize, css:piCurrent.touchAreaCss,
					location:{right:0}, data:{handle:'right'}, nolog:true},
				{media:{html:'<div></div>'}, size:piCurrent.touchAreaSize, css:piCurrent.touchAreaCss,
					location:{left:0}, data:{handle:'left'}, nolog:true}
			]
		});
		...
		function getInstHTML(params)
		{
			...
			{
				instHTML = getInstFromTemplate(isTouch ? piCurrent.instTemplatePracticeTouch : piCurrent.instTemplatePractice, params.blockNum, params.nBlocks);
			}
			else if (params.categorySide == 'rightCat')
			{
				instHTML = getInstFromTemplate(isTouch ? piCurrent.instTemplateCategoryRightTouch : piCurrent.instTemplateCategoryRight, params.blockNum, params.nBlocks);
			}
			else if (params.categorySide == 'leftCat')
			{
				instHTML = getInstFromTemplate(isTouch ? piCurrent.instTemplateCategoryLeftTouch : piCurrent.instTemplateCategoryLeft, params.blockNum, params.nBlocks);
			}
			return (instHTML);
		}
		//On touch the instructions sit below the category labels, so they need an explicit
		//top. On the keyboard version we keep the original bottom-anchored placement.
		function getInstLocation(isPractice)
		{
			if (!isTouch) {return undefined;}
			return {left:0, top: isPractice ? piCurrent.touchInstTopPractice : piCurrent.touchInstTopCombined};
		}
		...
		{//For each block
			...
			var instLocation = getInstLocation(isPrac);
			if (instLocation) {instStim.location = instLocation;}
			trialSequence.push(
				{
					inherit : 'instructions',
					data: {blockStart:true},
					layout : blockLayout, //No green response areas here: the bottom bar is the control.
					stimuli : [
						instStim,
						{
					...
					[
						{//The single attribute trials
							...
							[{
								...
								layout : blockLayout.concat(trialExtras)
							}]
						}, 
		...
		var finalStim = {
			...
			media:{html:'<div><p style="font-size:' + (isTouch ? '20px' : '28px') + '"><color="#000000">' +
			(isTouch ? piCurrent.finalTouchText : piCurrent.finalText) + '</p></div>'}
		};
		if (isTouch) {finalStim.location = {left:0, top:20};}
		trialSequence.push({
			inherit : 'instructions',
			data: {blockStart:true},
			layout : [{media:{word:''}}],
