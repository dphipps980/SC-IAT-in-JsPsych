...
window.jsPsych6 = (function() {
  ...
  core.init = function(options) {
    ...
    if(typeof options.timeline === 'undefined'){
      console.error('No timeline declared in jsPsych6.init. Cannot start experiment.')
    }
    ...
    // set DOM element where jsPsych6 will render content
    // if undefined, then jsPsych6 will use the <body> tag and the entire page
    if(typeof opts.display_element == 'undefined'){
      ...
      if(display === null) {
        console.error('The display_element specified in jsPsych6.init() does not exist in the DOM.');
      } else {
        opts.display_element = display;
      }
    }
    opts.display_element.innerHTML = '<div class="jsPsych6-content-wrapper"><div id="jsPsych6-content"></div></div>';
    DOM_container = opts.display_element;
    DOM_target = document.querySelector('#jsPsych6-content');
    ...
    if(opts.display_element.className.indexOf('jsPsych6-display-element') == -1){
      opts.display_element.className += ' jsPsych6-display-element';
    }
    DOM_target.className += 'jsPsych6-content';
    ...
    jsPsych6.pluginAPI.initAudio();

    // below code resets event listeners that may have lingered from
    // a previous incomplete experiment loaded in same DOM.
    jsPsych6.pluginAPI.reset(opts.display_element);
    // create keyboard event listeners
    jsPsych6.pluginAPI.createKeyboardEventListeners(opts.display_element);
    // create listeners for user browser interaction
    jsPsych6.data.createInteractionListeners();
    ...
    checkExclusions(opts.exclusions,
      function(){
        ...
        if(opts.auto_preload){
          jsPsych6.pluginAPI.autoPreload(timeline, startExperiment, opts.preload_images, opts.preload_audio, opts.preload_video, opts.show_preload_progress_bar);
  ...
  core.finishTrial = function(data) {
    ...
    jsPsych6.data.write(data);

    // get back the data with all of the defaults in
    var trial_data = jsPsych6.data.get().filter({trial_index: global_trial_index});
  ...
  core.endExperiment = function(end_message) {
    ...
    timeline.end();
    jsPsych6.pluginAPI.cancelAllKeyboardResponses();
  ...
  core.addNodeToEndOfTimeline = function(new_timeline, preload_callback){
    ...
    if(typeof preload_callback !== 'undefinded'){
      if(opts.auto_preload){
        jsPsych6.pluginAPI.autoPreload(timeline, preload_callback);
      } else {
        preload_callback();
      }
    }
  ...
  function TimelineNode(parameters, parent, relativeID) {
    ...
    this.trial = function() {
      if (typeof timeline_parameters == 'undefined') {
        ...
        // protect functions.
        return jsPsych6.utils.deepCopy(trial_parameters);
      } else {
    ...
    this.setTimelineVariablesOrder = function() {
      ...
      if(typeof timeline_parameters.sample !== 'undefined'){
        if(timeline_parameters.sample.type == 'custom'){
          ...
          order = jsPsych6.randomization.sampleWithReplacement(order, timeline_parameters.sample.size, timeline_parameters.sample.weights);
        } else if(timeline_parameters.sample.type == 'without-replacement'){
          order = jsPsych6.randomization.sampleWithoutReplacement(order, timeline_parameters.sample.size);
        } else if(timeline_parameters.sample.type == 'fixed-repetitions'){
          order = jsPsych6.randomization.repeat(order, timeline_parameters.sample.size, false);
        } else if(timeline_parameters.sample.type == 'alternate-groups'){
          order = jsPsych6.randomization.shuffleAlternateGroups(timeline_parameters.sample.groups, timeline_parameters.sample.randomize_group_order);
        } else {
      ...
      if(timeline_parameters.randomize_order) {
        order = jsPsych6.randomization.shuffle(order);
      }
    ...
    this.generatedData = function() {
      return jsPsych6.data.getDataByTimelineNode(this.ID());
    }
    ...
    var _construct = function() {
      ...
      if ((typeof parameters.timeline !== 'undefined') || (typeof jsPsych6.plugins[trial_type] == 'function')) {
      ...
      // if there is no timeline parameter, then this node is a trial node
        ...
        } else if ((typeof jsPsych6.plugins[trial_type] == 'undefined') && (trial_type.toString().replace(/\s/g,'') != "function(){returntimeline.timelineVariable(varname);}")) {
  ...
  function finishExperiment() {
    ...
    opts.on_finish(jsPsych6.data.get());
  ...
  function doTrial(trial) {
    ...
    jsPsych6.plugins[trial.type].trial(DOM_target, trial);
  ...
  function evaluateFunctionParameters(trial){
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      if(keys[i] !== 'type'){
        ...
        if(
          (typeof jsPsych6.plugins.universalPluginParameters[keys[i]] !== 'undefined' && jsPsych6.plugins.universalPluginParameters[keys[i]].type !== jsPsych6.plugins.parameterType.FUNCTION ) ||
          (typeof jsPsych6.plugins[trial.type].info.parameters[keys[i]] !== 'undefined' && jsPsych6.plugins[trial.type].info.parameters[keys[i]].type !== jsPsych6.plugins.parameterType.FUNCTION)
        ) {
  ...
  function setDefaultValues(trial){
    for(var param in jsPsych6.plugins[trial.type].info.parameters){
      // check if parameter is complex with nested defaults
      if(jsPsych6.plugins[trial.type].info.parameters[param].type == jsPsych6.plugins.parameterType.COMPLEX){
        if(jsPsych6.plugins[trial.type].info.parameters[param].array == true){
          // iterate over each entry in the array
          for(var i in trial[param]){
            // check each parameter in the plugin description
            for(var p in jsPsych6.plugins[trial.type].info.parameters[param].nested){
              if(typeof trial[param][i][p] == 'undefined' || trial[param][i][p] === null){
                if(typeof jsPsych6.plugins[trial.type].info.parameters[param].nested[p].default == 'undefined'){
                  console.error('You must specify a value for the '+p+' parameter (nested in the '+param+' parameter) in the '+trial.type+' plugin.');
                } else {
                  trial[param][i][p] = jsPsych6.plugins[trial.type].info.parameters[param].nested[p].default;
                }
              }
            }
          }
      ...
      // if it's not nested, checking is much easier and do that here:
        ...
        if(typeof jsPsych6.plugins[trial.type].info.parameters[param].default == 'undefined'){
          console.error('You must specify a value for the '+param+' parameter in the '+trial.type+' plugin.');
        } else {
          trial[param] = jsPsych6.plugins[trial.type].info.parameters[param].default;
        }
      }
    }
  }
  ...
  function drawProgressBar(msg) {
    document.querySelector('.jsPsych6-display-element').insertAdjacentHTML('afterbegin',
      '<div id="jsPsych6-progressbar-container">'+
      '<span>'+
      msg+ 
      '</span>'+
      '<div id="jsPsych6-progressbar-outer">'+
        '<div id="jsPsych6-progressbar-inner"></div>'+
      '</div></div>');
  }

  function updateProgressBar() {
    var progress = jsPsych6.progress().percent_complete;
  ...
  core.setProgressBar = function(proportion_complete){
    ...
    document.querySelector('#jsPsych6-progressbar-inner').style.width = (proportion_complete*100) + "%";
  ...
  //Leave a trace in the DOM that jsPsych6 was loaded
  document.documentElement.setAttribute('jsPsych6', 'present');

  return core;
})();

jsPsych6.plugins = (function() {
...
jsPsych6.data = (function() {
  ...
  function DataCollection(data){
    ...
    data_collection.readOnly = function(){
      return DataCollection(jsPsych6.utils.deepCopy(trials));
    }
    ...
    data_collection.filter = function(filters){
      ...
      if(!Array.isArray(filters)){
        var f = jsPsych6.utils.deepCopy([filters]);
      } else {
        var f = jsPsych6.utils.deepCopy(filters);
      }
    ...
    data_collection.ignore = function(columns){
      ...
      var o = jsPsych6.utils.deepCopy(trials);
  ...
  module.write = function(data_object) {
    ...
    var progress = jsPsych6.progress();
    var trial = jsPsych6.currentTrial();
    ...
    var default_data = {
      ...
      'time_elapsed': jsPsych6.totalTime(),
      'internal_node_id': jsPsych6.currentTimelineNodeID()
    };
  ...
  module.displayData = function(format) {
    ...
    var display_element = jsPsych6.getDisplayElement();

    display_element.innerHTML = '<pre id="jsPsych6-data-display"></pre>';

    document.getElementById('jsPsych6-data-display').textContent = data_string;
  };
  ...
  module.createInteractionListeners = function(){
    ...
    window.addEventListener('blur', function(){
      var data = {
        event: 'blur',
        trial: jsPsych6.progress().current_trial_global,
        time: jsPsych6.totalTime()
      };
      interactionData.push(data);
      jsPsych6.initSettings().on_interaction_data_update(data);
    });
  ...
  function saveTextToFile(textstr, filename) {
    ...
    display_element.insertAdjacentHTML('beforeend','<a id="jsPsych6-download-as-text-link" style="display:none;" download="'+filename+'" href="'+blobURL+'">click to download</a>');
    document.getElementById('jsPsych6-download-as-text-link').click();
  }
...
jsPsych6.turk = (function() {
  ...
  module.submitToTurk = function(data) {
    ...
    var turkInfo = jsPsych6.turk.turkInfo();
...
jsPsych6.randomization = (function() {
  ...
  module.shuffle = function(arr) {
    if(!Array.isArray(arr)){
      console.error('Argument to jsPsych6.randomization.shuffle() must be an array.')
    }
    return shuffle(arr);
  }
  ...
  module.shuffleNoRepeats = function(arr, equalityTest) {
    if(!Array.isArray(arr)){
      console.error('First argument to jsPsych6.randomization.shuffleNoRepeats() must be an array.')
    }
    if(typeof equalityTest !== 'undefined' || typeof equalityTest !== 'function'){
      console.error('Second argument to jsPsych6.randomization.shuffleNoRepeats() must be a function.')
    }
  ...
  module.shuffleAlternateGroups = function(arr_groups, random_group_order){
    ...
    if(n_groups == 1){
      console.warn('jsPsych6.randomization.shuffleAlternateGroups was called with only one group. Defaulting to simple shuffle.');
  ...
  module.sampleWithoutReplacement = function(arr, size){
    if(!Array.isArray(arr)){
      console.error("First argument to jsPsych6.randomization.sampleWithoutReplacement() must be an array")
    }
    ...
    return jsPsych6.randomization.shuffle(arr).slice(0,size);
  }

  module.sampleWithReplacement = function(arr, size, weights) {
    if(!Array.isArray(arr)){
      console.error("First argument to jsPsych6.randomization.sampleWithReplacement() must be an array")
    }
...
jsPsych6.pluginAPI = (function() {
  ...
  module.getKeyboardResponse = function(parameters) {
    ...
    var listener_function = function(e) {
      ...
      if (typeof parameters.valid_responses === 'undefined' || parameters.valid_responses == jsPsych6.ALL_KEYS) {
        valid_response = true;
      } else {
        if(parameters.valid_responses != jsPsych6.NO_KEYS){
          for (var i = 0; i < parameters.valid_responses.length; i++) {
            if (typeof parameters.valid_responses[i] == 'string') {
              var kc = jsPsych6.pluginAPI.convertKeyCharacterToKeyCode(parameters.valid_responses[i]);
  ...
  module.initAudio = function(){
    context = (jsPsych6.initSettings().use_webaudio === true) ? jsPsych6.webaudio_context : null;
  }
  ...
  module.preloadAudioFiles = function(files, callback_complete, callback_load) {
    ...
    files = jsPsych6.utils.flatten(files);
    files = jsPsych6.utils.unique(files);
    ...
    function load_audio_file_webaudio(source, count){
      ...
      request.onerror = function(){
        if(count < jsPsych6.initSettings().max_preload_attempts){
          setTimeout(function(){
  ...
  module.preloadImages = function(images, callback_complete, callback_load) {
    ...
    images = jsPsych6.utils.flatten(images);
    images = jsPsych6.utils.unique(images);
    ...
    module.preloadVideo = function(video, callback_complete, callback_load) {
        ...
        // flatten the images array
        video = jsPsych6.utils.flatten(video);
        video = jsPsych6.utils.unique(video);
  ...
  module.registerPreload = function(plugin_name, parameter, media_type, conditional_function) {
    if (['audio', 'image', 'video'].indexOf(media_type)===-1) {
      console.error('Invalid media_type parameter for jsPsych6.pluginAPI.registerPreload. Please check the plugin file.');
    }
  ...
  module.autoPreload = function(timeline, callback, images, audio, video, progress_bar) {
      ...
      for (var j = 0; j < trials.length; j++) {
        ...
        if (trials[j][param] && typeof trials[j][param] !== 'function') {
          ...
          if ( !func  || func(trials[j]) ){
            if (media === 'image') {
              images = images.concat(jsPsych6.utils.flatten([trials[j][param]]));
            } else if (media === 'audio') {
              audio = audio.concat(jsPsych6.utils.flatten([trials[j][param]]));
            }
            else if (media === 'video') {
              video = video.concat(jsPsych6.utils.flatten([trials[j][param]]));
            }
          }
        }
      }
    ...
    images = jsPsych6.utils.unique(jsPsych6.utils.flatten(images));
    audio  = jsPsych6.utils.unique(jsPsych6.utils.flatten(audio));
    video  = jsPsych6.utils.unique(jsPsych6.utils.flatten(video));
    ...
    if(progress_bar){
      var pb_html = "<div id='jsPsych6-loading-progress-bar-container' style='height: 10px; width: 300px; background-color: #ddd;'>";
      pb_html += "<div id='jsPsych6-loading-progress-bar' style='height: 10px; width: 0%; background-color: #777;'></div>";
      pb_html += "</div>";
      jsPsych6.getDisplayElement().innerHTML = pb_html;
    }
    ...
    function update_loading_progress_bar(){
      ...
      if(progress_bar){
        ...
        jsPsych6.getDisplayElement().querySelector('#jsPsych6-loading-progress-bar').style.width = percent_loaded+"%";
      }
    }
  ...
  module.hardware = function hardware(mess){
	  //since Chrome extension content-scripts do not share the javascript environment with the page script that loaded jsPsych6,
	  //we will need to use hacky methods like communicating through DOM events.
	  var jsPsych6Evt = new CustomEvent('jsPsych6', {detail: mess});
	  document.dispatchEvent(jsPsych6Evt);
  ...
  /** {boolean} Indicates whether this instance of jsPsych6 has opened a hardware connection through our browser extension */
  ...
  document.addEventListener("jsPsych6-activate", function(evt){
